...
export class EmployerService {
  ...
  getEmployers(): Observable<Employer[]> {
	// ...using get request
  	return this.http.get('http://localhost:3000/employers.json', this.options)
  ...
  createEmployer(data): Observable<Employer[]> {
    ...
    let user = atob(this.user);
  	...
  	return this.http.post('http://localhost:3000/employers.json', {"employer":{"legal_name":legal_name, "hbx_id":hbx_id, "dba":dba, "fein":fein, "user":user, "is_active":true}}, this.options)
  ...
  deleteEmp(data): Observable<Employer[]> {
      return this.http.delete('http://localhost:3000/employers/'+data.id+'.json', this.options)
  	  // ...and calling .json() on the response to set data
